 ...
 * There is NO genericOAuth client and NO pre-login sign-out.
 */
...
export const GUEST_PLAY_KEY = "jarvis-guest-play";
 ...
 * Production: full-page redirect via POST /api/auth/sign-in/social.
 * Live preview iframe: popup (cannot top-level redirect inside the embed).
 *
 * Never signs the user out first. Never retries. Never uses generic OAuth.
 */
export async function signIn(
  ...
  if (inLivePreview()) {
    const popup = openSignInPopup("google");
...
function openSignInPopup(providerId: string): Window | null {
  ...
  const name = `jarvis-signin-${Date.now()}`;
